...
import { z } from "zod";
import { COMPONENTS, CATEGORIES } from "./data.js";
...
function formatComponent(c, verbose = false) {
    ...
    if (verbose) {
        if (c.plain_what)
            lines.push(``, `**What it does:** ${c.plain_what}`);
        if (c.plain_why)
            lines.push(`**Why use it:** ${c.plain_why}`);
        if (c.common_use)
            lines.push(`**Common use:** ${c.common_use}`);
        lines.push(``);
        if (c.tags.length)
            lines.push(`**Tags:** ${c.tags.join(", ")}`);
        if (c.related.length)
            lines.push(`**Often used with:** ${c.related.join(", ")}`);
        lines.push(``);
        if (c.github_url)
            lines.push(`**GitHub:** ${c.github_url}`);
        if (c.docs_url)
            lines.push(`**Docs:** ${c.docs_url}`);
        ...
        const links = [];
        if (c.github_url)
            links.push(`[GitHub](${c.github_url})`);
        if (c.docs_url)
            links.push(`[Docs](${c.docs_url})`);
...
server.registerTool("codefest_search_components", {
    ...
    if (query) {
        ...
        results = results
            ...
            if (c.name.toLowerCase().includes(q))
                score += 10;
            if (c.tags.some(t => t.includes(q)))
                score += 5;
            if (c.description.toLowerCase().includes(q))
                score += 2;
            if (c.common_use.toLowerCase().includes(q))
                score += 1;
            return { c, score };
        })
...
// ── Tool: get_component ───────────────────────────────────────────────────────
server.registerTool("codefest_get_component", {
...
server.registerTool("codefest_list_categories", {
...
server.registerTool("codefest_recommend_stack", {
...
Returns: Ordered stack list with setup times, total time estimate, and setup advice.`,
    inputSchema: z.object({
        action: z.enum(["map", "form", "ai", "data", "realtime", "alerts"]).describe("Problem type: map=geographic data, form=user input, ai=LLM features, data=visualization, realtime=live collab, alerts=notifications"),
    ...
    const stack = stackNames.map(name => {
        ...
        return comp ?? { name, setup_time_minutes: 0, github_url: "", docs_url: "", description: "", category: "", difficulty: "beginner" };
    });
    ...
    stack.forEach((s, i) => {
        const links = [];
        if (s.github_url)
            links.push(`[GitHub](${s.github_url})`);
        if (s.docs_url)
            links.push(`[Docs](${s.docs_url})`);
        if ("tags" in s)
            links.push(`[Codefest](${componentUrl(s.name)})`);
...
server.registerTool("codefest_generate_context", {
